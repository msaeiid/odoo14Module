    ...
    const DocumentsInspector = Widget.extend({
        ...
        init: function (parent, params) {
            ...
            this._isLocked = this.records.some(record =>
                // ##ASSANIDEH## added this line to the source && ! session.is_admin
                record.data.lock_uid && (record.data.lock_uid.res_id !== session.uid) && !session.is_admin
            );
        ...
        async _renderHistory() {
            ...
            // ##ASSANIDEH## Two following lines are added to hide restore and delete button in history
            this.$('.o_inspector_history_buttons .o_inspector_history_item_delete').prop('hidden', this._isLocked);
            this.$('.o_inspector_history_buttons  .o_inspector_history_item_restore').prop('hidden', this._isLocked);
        },
        /**
         * @private
         */
